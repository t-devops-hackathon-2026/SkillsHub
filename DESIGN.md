...
本書は**段階リリース（Step 1〜3）**を前提に構成する。Step 1 のダッシュボード（可視化）と提案（重複・合成・更新）をミニマム目標とし、改善提案・利用記録は後続ステップで足す。各機能の所属ステップは次章「開発フェーズ」を参照。**ダッシュボードはログイン不要の閲覧ツール**とし、投票・コメント・個人ビューといった GitHub で代替できる機能は持たない（Skill への要望・議論はソースリポジトリの GitHub issue で行う）。
...
仕様確定時の議論を踏まえ、機能を3段階に分けて段階的に実装する。**Step 1（可視化＋提案コア）を最優先のミニマム目標**とする。**いずれのステップでもダッシュボードはログイン不要**（投票・コメント・個人ビューを持たず、GitHub への書き戻しもしないため、ユーザー認証は設けない）。

| Step | テーマ | 含む機能 |
|---|---|---|
| **Step 1** | 可視化 ＋ 提案コア（MVP） | 司書による自律収集・解析・品質採点・鮮度判定（コミット経過時間のみ）・重複検出、ダッシュボード一覧/絞り込み/サマリ、自然言語検索、Skill詳細、リポジトリ登録、提案レビュー（**重複 merge / 合成 compose / 更新 update** の生成・表示・採用記録） |
| **Step 2** | 改善提案 | **改善 improve**（弱い観点を補う diff 改善提案）の追加 |
| **Step 3** | 利用記録 ＋ 鮮度精緻化 | hooks オプトインによる利用回数の**匿名・非同期**収集、鮮度判定をコミット経過時間＋利用回数の2軸へ拡張 |

方針の要点:

- **ログイン不要の閲覧ツールにする**。投票・コメント・個人ビュー（自分の作者Skill一覧など）は設けない。需要・人気の可視化は Step 3 の利用回数に一本化し、Skill への要望・議論はソースの GitHub issue に寄せる（GitHub を見られる人＝その Skill を使える人、という前提が成り立つため自然）。
- **提案はアプリ内で完結**（GitHub への自動 issue/PR 化はしない）。Step 1 で重複・合成・更新を出し、診断的な改善提案（improve）は Step 2 で足す。採用/却下はステータス記録のみ。
- **利用記録は Step 3・匿名・オプトイン**。鮮度判定の第2軸（利用回数）に使う。「誰が使ったか」は持たず回数だけを集計する。収集口の信頼方式は Step 3 着手時に確定する（候補: GCS へのアップロード回収 / 共有トークン付き収集API / GitHub トークンでのゲート。詳細は後述「利用記録の収集」）。
- **通知機構は対象外（将来）**。「要更新」に気づく手段は当面、社内なので Slack メンション等の人的声がけで代替する。

> この段階分けにより、Step 1 単体でも「散在 Skill を集めて可視化し、重複・陳腐化を自動で指摘し、合成・更新まで提案する」デモが成立する（審査基準①②③④を充足）。Step 2/3 は体験価値を積み増す拡張と位置づける。
...
flowchart LR
    ...
    Detail["③ Skill詳細<br/>使い方/スコア内訳/提案"]
...
| データ基盤 | Cloud SQL for PostgreSQL + pgvector | Skill・提案の永続化（利用記録はStep3）と、ベクトル類似検索を1つのDBで完結させる | 通常データとベクトルを一本化 |
...
| Cloud SQL + pgvector | Skills本体・埋め込み・提案の永続化（利用記録は Step 3） |
...
- 絞り込みツールバー: フリーワード、鮮度（new/stale/needs_update）、タグ、ソート（品質/更新日）。
- Skillカードグリッド: 鮮度バッジ・品質スコア・タグ・作者・取得元リンク。クリックで詳細へ。
...
- 提案: 対象 Skill に紐づく open な Suggestion（重複/合成/更新。改善 improve は Step 2。diff 下書き含む）。承認/却下可。
- 議論・要望: 独自のコメント機能は持たず、取得元リポジトリの GitHub issue へ誘導するリンクを置く。
...
> エンティティの所属ステップ: **Step 1** = `Repository` / `Skill` / `Skill_Embedding` / `Suggestion` / `Suggestion_Target`。**Step 3** = `Usage_Event`。Step 2（改善提案）は既存の `Suggestion`（`type='improve'`）を使うため新テーブルは不要。
>
> PRDからの差分: 独自の `Comment`・`Vote`・`User` は廃止（投票・コメント・個人ビュー・ログインを持たないため）。要望は GitHub issue、需要は Step 3 の `Usage_Event`（匿名・回数集計）で代替する。`Repository` を追加（登録制収集の単位）、`Skill.usage/quality_breakdown/content_hash` を追加、`Suggestion` の多対多をブリッジ表 `suggestion_target` で表現（compose型が複数Skillを参照するため）。
...
flowchart TB
    ...
    subgraph Online["オンライン: 対話 (アプリから起動)"]
        ...
        Improver["ImproverAgent<br/>diff改善提案 improve<br/>(output_schema) Step2"]
...
- **提案とステップの対応**: 重複（Dedup→merge）・合成（Composer→compose）・更新（update＝鮮度 `needs_update` への diff）は **Step 1**。改善（Improver→improve）は **Step 2**。update の diff 生成は鮮度判定パイプラインの一部として Step 1 に含め、Improver は Step 2 で追加する。
...
### 鮮度判定
Step 1 ではコミット経過時間を主軸に、Analyzer（鮮度判定ステップ）が判定する。
...
- Step 3 で「利用回数」を加えた2軸判定に拡張する（後述「利用記録の収集」）。
...
### 提案の採用時挙動
- `merge`/`compose`/`improve`: ステータスを `accepted` に更新（記録のみ。GitHubへの自動 issue/PR 化はしない方針）。`improve` は Step 2。
- `update`: diffを「適用済みドラフト」として記録し、対象Skillの `freshness_status` を `new` に戻す（実コミットは作者が手元で実施）。
- いずれも監査のため `accepted` 履歴（種別・対象・日時）を残す。ログインを設けないため「誰が」までは記録しない。

---

## 利用記録の収集（Step 3）

鮮度判定の第2軸（利用回数）に使う「どの Skill がどれだけ使われているか」を収集する。プライバシーと負荷に配慮し、**匿名・オプトイン・最小データ**に限定する。

- **収集範囲は最小**: 「どの Skill を使ったか」（`skill_name` / `used_at`）のみ。**誰が使ったかは持たない（匿名・回数集計）**。Skill の実行内容や入出力は一切収集しない。
- **オプトイン**: 各利用者が自分の環境で hooks を設定したときだけ送信される。使いたい人だけが設定すればよい。
- **送信方式（負荷・受け口への配慮）**: Skill 実行のたびに同期 HTTP を叩く方式は、通信オーバーヘッドと受け口の常時開放という難点がある（Skill によっては高頻度実行もありうる）。そこで **ローカルにイベントをバッファし、日次などでまとめて送る**非同期方式を基本とする。これにより実行のホットパスから通信を外し、公開する受け口も最小化する。
- **収集口の信頼方式（Step 3 で確定）**: ダッシュボード自体はログイン不要だが、収集口だけは「誰でも書き込める」状態を避けたい。匿名集計が前提なので本人確認まではせず、受け口を絞る方向を推奨する。候補は次の3つ。
  - **GCS へアップロード回収（推奨）**: 収集APIを公開せず、hooks はサービスキー/署名URLで GCS にイベントファイルを投下、司書バッチが回収する。受け口の攻撃面が最小。
  - **共有トークン付き収集API**: 軽量な収集APIを立て、配布した共有トークンで受理。実装は素直だが受け口を常時公開する。
  - **GitHub トークンでのゲート**: org メンバーに絞りたい場合のみ、hooks の GitHub トークンを収集側で検証する。本人確認は要らない（匿名集計）ため通常は過剰。
...
> 段階方針: まずコミット経過時間だけで動く Step 1 の鮮度判定を実装し、利用回数は本ステップで後付けする2段構えとする。
