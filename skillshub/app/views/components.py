...
def to_jst(dt: datetime) -> datetime:
    """DB に UTC で保存された時刻を日本時間（UTC+9）へずらして表示に使う。

    読み出し値は naive（tz 情報なしの UTC）で来るため、tz 変換 API ではなく
    単純に 9 時間足す。表示専用で、比較・保存には使わない。
    """
...
/* パスワード表示切り替え（目のアイコン）も出さない。
   アイコン用の右 padding も詰めて、入力欄とログインボタンの幅を揃える */
...
[class*="st-key-login_card"] [data-testid="stTextInputRootElement"]{padding-right:0}
...
/* Enter 送信用に form で包んでいるため、フォームのカード枠・ヒントは出さない */
[class*="st-key-agent_bar_box"] [data-testid="stForm"]{
  background:transparent;border:0;box-shadow:none;padding:0}
[class*="st-key-agent_bar_box"] [data-testid="InputInstructions"]{display:none}
[class*="st-key-agent_bar_box"] [data-testid="stFormSubmitButton"] button{
  background:var(--gh-grad);color:#fff;border:0;font-weight:600;box-shadow:var(--gh-shadow-2)}
[class*="st-key-agent_bar_box"] [data-testid="stFormSubmitButton"] button:hover{
...
/* 登録済みテーブルのヘッダー行：データ行のカード（border 1px + padding 1rem）と
   桁位置を揃えるため、同じ水平オフセットを与える。 */
.stVerticalBlock[class*="st-key-repo_table_header"]{padding:0 calc(1rem + 1px)}

/* ── 収集元ページのローディングオーバーレイ ──
   GitHub スコープ取得（初回・TTL 切れ時に数秒ブロック）の間、前画面の stale 要素が
   透けて見えないようメイン領域だけを覆う。サイドバーは操作の拠り所として見せたまま残す。
   表示・解除は repos.render() の st.empty() が行う。 */
/* オーバーレイの absolute の基準をメイン領域にする。直近の親 stElementContainer が
   relative を持つため、オーバーレイを含むものだけ static に外す。 */
[data-testid="stMain"]{position:relative}
[data-testid="stElementContainer"]:has(.sh-loading-overlay){position:static}
.sh-loading-overlay{position:absolute;inset:0;z-index:9999;background:var(--gh-bg)}
/* スピナーはスクロール位置によらず見えている高さの中央に出す（sticky） */
.sh-loading-overlay .sh-loading-panel{position:sticky;top:0;height:100vh;
  display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px}
.sh-loading-overlay .sh-loading-spinner{width:30px;height:30px;border-radius:50%;
  border:3px solid var(--gh-line);border-top-color:var(--gh-accent);
  animation:sh-spin .8s linear infinite}
.sh-loading-overlay .sh-loading-text{color:var(--gh-muted);font-size:.9rem;font-weight:500}
...
/* サイドバーのデモリセット：Primer btn-danger 風。
   トリガーは outline、確認ダイアログ内の実行ボタンはベタ塗りにする。
   ダイアログの中身は modal に描画されサイドバー外のため、実行ボタンは key だけで指す。 */
[data-testid="stSidebar"] div[class*="st-key-demo_reset_open"] button{
...
[data-testid="stSidebar"] div[class*="st-key-demo_reset_open"] button:hover{
  background:var(--gh-danger);border-color:var(--gh-danger);color:#fff}
[data-testid="stSidebar"] div[class*="st-key-demo_reset_open"] button p{
  text-align:center;width:100%}
/* 実行ボタン：Primer btn-danger のベタ塗り。ダイアログを開くと自動フォーカスが
   当たり、Streamlit 既定の focus スタイル（emotion クラス＋擬似クラスの組み合わせで
   詳細度が高い）が背景を薄グレーに戻して白文字が溶けるため、!important で
   focus・active・disabled（リセット実行中）を含む全状態を赤に固定する。 */
div[class*="st-key-demo_reset_confirm"] button{
  background:var(--gh-danger) !important;border:1px solid rgba(31,35,40,.15) !important;
  color:#fff !important;font-weight:600}
div[class*="st-key-demo_reset_confirm"] button:hover{
  background:var(--gh-danger-h) !important;border-color:var(--gh-danger-h) !important}
div[class*="st-key-demo_reset_confirm"] button:disabled{
  background:var(--gh-danger) !important;color:rgba(255,255,255,.75) !important}
